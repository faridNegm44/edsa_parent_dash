...
trait ParentProblemDatatableTrait{

    public function getParentProblemDatatable($problemStatus = null, $deadline = false)
    {   
        if($problemStatus){
            if (auth()->user()->user_status == 1) {
                $all = ParentProblems::where('problem_status', $problemStatus)->get();
    
            } elseif (auth()->user()->user_status == 2 || auth()->user()->user_status == 4) {
                $all = ParentProblems::where('problem_status', $problemStatus)
                                    ->where(function($q){
