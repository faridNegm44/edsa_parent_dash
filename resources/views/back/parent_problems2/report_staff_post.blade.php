...
@section('title') تقرير تقيم موظف @endsection
...
@section('content')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="left-content">
            <h4 class="content-title mb-2">تقرير تقيم موظف</h4>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ url('dashboard') }}">الرئيسيه</a></li>
                    <li class="breadcrumb-item active" aria-current="page">تقرير تقيم موظف</li>
                </ol>
            </nav>
        </div>
    </div>
            ...
            <div class="card-body">
                <div class="panal_body" style="padding: 40px 20px;">
                    ...
                    @if (count($find) != null)
                                        ...
                                        <td>
                                            <ul>
                                                <li>
                                                    ...
                                                    {{-- <span class="badge badge-dark" style="font-size: 15px;"> {{ $item->problem_rating }}</span> --}}
                                                    {{ $item->problem_rating }}
                                                </li>
                                                <hr>
                                                <li>
                                                    <span style="font-weight: bold;font-size: 12px;">حالة الطلب :</span><br />
                                                    {{-- <span class="badge badge-dark" style="font-size: 15px;"> {{ $item->problem_status }}</span> --}}
                                                    {{ $item->problem_status }}
                                                </li>
                                            </ul>
                                        </td>
                                        <td>
                                            <ul>
                                                <li>
                                                    ...
                                                    <span style="font-size: 15px;"> {{ $item->date_reference == null ? ' ' : Carbon\Carbon::parse($item->date_reference)->format('Y-m-d') }}</span>
                                                    <br />
                                                    <span style="font-size: 15px;"> {{ $item->date_reference == null ? ' ' : Carbon\Carbon::parse($item->date_reference)->format('h:i') }}</span>
                                                </li>
                                            </ul>
                                        </td>
                                        <td>
                        ...
                        <br>
                        @if ($count_problems_between_dates == null)
                            <div class="alert alert-danger text-center">
                                لإظهار النسبة المئويه الخاصة بالموظف يجب تحديد تاريخ من وإلي
                            </div>
                        @else
                            <table class="table table-bordered text-center">
                                <thead class="text-white">
                                    <tr>
                                        ...
                                        <td>إجمالي الطلبات المسندة للموظف</td>
                                        <td>نسبة المهام الموكلة</td>
                                        <td>مدي صعوبة المهمات</td>
                                        <td>سرعة وجودة الأداء</td>
                                ...
                                <tbody>
                                    <tr>
                                        <td>{{ $count_problems_between_dates }}</td>
                                        <td>{{ $count_problems_assigned_to_staff }}</td>
                                        <td>{{ $percentage_of_assigned_tasks }} %</td>
                                        <td>{{ sprintf("%.2f", $problems_ratings/$count_problems_assigned_to_staff) }} %</td>
                                        <td>{{ sprintf("%.2f", $staff_ratings/$count_problems_assigned_to_staff) }} %</td>
                                        <td style="color: red;font-weight: bold;font-size: 22px;">
                                            {{ sprintf("%.2f", ($percentage_of_assigned_tasks+($problems_ratings/$count_problems_assigned_to_staff)+$staff_ratings/$count_problems_assigned_to_staff) / 300 *100 )}}
                                            %
                                        </td>
                                    </tr>
                                </tbody>
