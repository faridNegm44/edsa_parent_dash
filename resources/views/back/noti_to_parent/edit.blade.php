            ...
            <div class="pd-30 pd-sm-40 bg-gray-100">
                <div class="row row-xs">
                    <div class="col-md-6">
                        ...
                        <select class="form-control select2_select2" id="parent_id" name="parent_id">
                            @foreach (\DB::table('tbl_parents')->get() as $parent)
                                    <option value="{{ $parent->ID }}" {{ $parent->ID == $first['parent_id'] ? 'selected' : null }} >
                                        {{ $parent->TheName0 }}
                                    </option>
                            @endforeach
                        </select>
...
<script>
    ...
    $(document).ready(function(){
        ...
        $(".modal #update").click(function(e) {
            e.preventDefault();
            let id = $('#res_id').val();            
            let act = "{{ url('dashboard/noti_to_parent/update') }}"+"/"+id;
