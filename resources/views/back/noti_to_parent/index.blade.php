...
@if(auth()->user()->user_status == 1 || auth()->user()->user_status == 2)
    @section('title') إشعارات لولي أمر أو أكثر @endsection
@elseif(auth()->user()->user_status == 3)
    @section('title') إشعارات @endsection
@endif
...
@section('footer')
    ...
    @if(auth()->user()->user_status == 1 || auth()->user()->user_status == 2)
        <script>
            $('#example2').DataTable({
                ...
                ajax: "{{ url('dashboard/noti_to_parent/datatable_noti_to_parent') }}",
                columns: [
                    {data: 'group_id', name: 'group_id'},
                    {data: 'parent_id', name: 'parent_name'},
    ...
    <script>
        $(document).on('click', '.change_readed', function(){
            ...
            let act = "{{ url('dashboard/noti_to_parent/change_readed') }}"+"/"+id;
    ...
    @include('back.noti_to_parent.delete');
...
@section('content')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="left-content">
            @if(auth()->user()->user_status == 1 || auth()->user()->user_status == 2)
                <h4 class="content-title mb-2">إشعارات لولي أمر أو أكثر</h4>
                @elseif(auth()->user()->user_status == 3)
                <h4 class="content-title mb-2">إشعارات </h4>
            @endif
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    @if(auth()->user()->user_status == 1 || auth()->user()->user_status == 2)
                        <li class="breadcrumb-item active" aria-current="page">إشعارات لولي أمر أو أكثر</li>
                    @elseif(auth()->user()->user_status == 3)
                        <li class="breadcrumb-item active" aria-current="page">إشعارات </li>
                    @endif
                ...
                <div class="d-flex justify-content-between">
                    ...
                    @if(auth()->user()->user_status == 1 || auth()->user()->user_status == 2)
                        <div class="col-xs-6 d-flex">
                            <a class="btn btn-primary btn-icon btn-sm text-white mr-2 modal-effect bt_modal" act="{{ url('dashboard/noti_to_parent/create') }}" data-effect="effect-sign" data-toggle="modal" href="#modaldemo8"><i class="fe fe-plus"></i></a>
                        </div>
                    @endif
